...
- [Implementation Steps](#implementation-steps)
  - [Step 1: Tone Mapping �?Map HDR to Displayable Range](#step-1-tone-mapping-map-hdr-to-displayable-range)
  - [Step 2: Gamma Correction �?Linear Space to Display Space](#step-2-gamma-correction-linear-space-to-display-space)
  ...
  - [Step 6: Gaussian Blur �?Basic Blur](#step-6-gaussian-blur-basic-blur)
  - [Step 7: Bloom �?HDR Glow](#step-7-bloom-hdr-glow)
  ...
  - [Step 12: FXAA �?Fast Approximate Anti-Aliasing](#step-12-fxaa-fast-approximate-anti-aliasing)
- [Variant Details](#variant-details)
  - [Variant 1: Multi-Pass Separable Bloom](#variant-1-multi-pass-separable-bloom)
  - [Variant 2: ACES + Complete Color Pipeline](#variant-2-aces-complete-color-pipeline)
  - [Variant 3: Physical DoF + Motion Blur Combination](#variant-3-physical-dof-motion-blur-combination)
...
- [In-Depth Performance Optimization](#in-depth-performance-optimization)
  - [1. Separable Blur Instead of 2D Convolution](#1-separable-blur-instead-of-2d-convolution)
  - [2. Hardware Mipmap Instead of Manual Downsampling](#2-hardware-mipmap-instead-of-manual-downsampling)
  - [3. Downsample Before Blurring](#3-downsample-before-blurring)
  - [4. Reduce Sample Count](#4-reduce-sample-count)
  - [5. Leverage Bilinear Interpolation for Free Blur](#5-leverage-bilinear-interpolation-for-free-blur)
  - [6. Conditional Compilation](#6-conditional-compilation)
  - [7. Avoid Branching](#7-avoid-branching)
- [Combination Suggestions](#combination-suggestions)
  - [1. Bloom + Tone Mapping (Most Basic Combination)](#1-bloom-tone-mapping-most-basic-combination)
  - [2. TAA + Motion Blur + DoF (Physical Camera Simulation)](#2-taa-motion-blur-dof-physical-camera-simulation)
  - [3. Chromatic Aberration + Vignette + Film Grain (Lens Simulation Trio)](#3-chromatic-aberration-vignette-film-grain-lens-simulation-trio)
  - [4. Color Grading + Tone Mapping + Contrast (Color Pipeline)](#4-color-grading-tone-mapping-contrast-color-pipeline)
  - [5. Bloom + Lens Flare (Cinematic Light Effects)](#5-bloom-lens-flare-cinematic-light-effects)
  - [6. Multi-Pass Complete Pipeline (Production-Grade)](#6-multi-pass-complete-pipeline-production-grade)
<!-- GENERATED:TOC:END -->
...
- GLSL fundamentals and the ShaderToy-style prototype environment (iResolution, iTime, iChannel, textureLod, etc.)
...
- Multi-pass rendering concepts (Buffer A/B/C/D and Image pass in ShaderToy-style prototype)
...
- Some pipelines use 0.4545 (�?/2.2) as the gamma value
...
- Note: ShaderToy-style prototype Buffers do not generate mipmaps by default; this must be enabled in the channel settings
...
`textureLod(tex, uv, lod)` leverages the GPU hardware's mipmap chain for free downsampled blur, suitable for fast bloom. Note that ShaderToy-style prototype Buffers do not generate mipmaps by default (you need to enable `mipmap` in the channel settings).
