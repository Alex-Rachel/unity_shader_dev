...
- [Prerequisites](#prerequisites)
  ...
  - [ShaderToy-style prototype Multi-Pass Mechanism](#shadertoy-style-prototype-multi-pass-mechanism)
  - [2D Grid Sampling](#2d-grid-sampling)
  - [Basic Vector Math](#basic-vector-math)
  - [Convolution Kernel Concepts](#convolution-kernel-concepts)
- [Implementation Steps in Detail](#implementation-steps-in-detail)
  - [Step 1: Grid State Storage and Self-Feedback](#step-1-grid-state-storage-and-self-feedback)
  - [Step 2: Initialization (Noise Seeding)](#step-2-initialization-noise-seeding)
  - [Step 3: Neighbor Sampling and Laplacian Computation](#step-3-neighbor-sampling-and-laplacian-computation)
  - [Step 4: State Update Rules](#step-4-state-update-rules)
  - [Step 5: Visualization and Coloring](#step-5-visualization-and-coloring)
- [Variant Details](#variant-details)
  - [Variant 1: Conway's Game of Life (Discrete CA)](#variant-1-conways-game-of-life-discrete-ca)
  - [Variant 2: Configurable Rule Set CA (Birth/Survival Bitmask)](#variant-2-configurable-rule-set-ca-birthsurvival-bitmask)
  - [Variant 3: Separable Gaussian Blur RD (Multi-Buffer Architecture)](#variant-3-separable-gaussian-blur-rd-multi-buffer-architecture)
  - [Variant 4: Continuous Differential Operator CA (Vein/Fluid Style)](#variant-4-continuous-differential-operator-ca-veinfluid-style)
  - [Variant 5: RD-Driven 3D Surface (Raymarched RD)](#variant-5-rd-driven-3d-surface-raymarched-rd)
- [Performance Optimization In-Depth Analysis](#performance-optimization-in-depth-analysis)
  - [1. texelFetch vs texture](#1-texelfetch-vs-texture)
  - [2. Separable Blur Instead of Large-Kernel Laplacian](#2-separable-blur-instead-of-large-kernel-laplacian)
  - [3. Multi-Step Sub-Iteration](#3-multi-step-sub-iteration)
  - [4. Reduced-Resolution Simulation](#4-reduced-resolution-simulation)
  - [5. Avoiding Branches and Conditionals](#5-avoiding-branches-and-conditionals)
- [Combination Suggestions �?Full Details](#combination-suggestions-full-details)
  - [1. RD + Raymarching (3D Displacement/Shaping)](#1-rd-raymarching-3d-displacementshaping)
  - [2. CA/RD + Particle Systems](#2-card-particle-systems)
  - [3. RD + Post-Processing Lighting](#3-rd-post-processing-lighting)
  - [4. CA + Color Decay Trails](#4-ca-color-decay-trails)
  - [5. RD + Domain Warping](#5-rd-domain-warping)
<!-- GENERATED:TOC:END -->
...
- **Multi-buffer feedback architecture**: ShaderToy-style prototype supports Buffer A~D, each buffer can bind itself or other buffers as iChannel input

### ShaderToy-style prototype Multi-Pass Mechanism
...
**What**: Use ShaderToy-style prototype's Buffer self-read mechanism to persistently store simulation state in a buffer texture. Each frame reads the previous frame's state, computes new state, and writes it back.
...
**Why**: Both CA and RD need initial perturbation to start evolution. Different initial conditions produce different final patterns. In practice, seeding is often repeated for the first 2~10 frames, since ShaderToy-style prototype occasionally skips the first frame.
...
| Separable 9-tap Gaussian | �?×9 | 18 |
| Separable 13-tap Gaussian | �?3×13 | 26 |
...
- Run the simulation at lower resolution in the buffer (not directly configurable in ShaderToy-style prototype, but possible in custom engines)
