...
- [Prerequisites](#prerequisites)
  ...
  - [SDF Concepts](#sdf-concepts)
  - [Numerical Differentiation](#numerical-differentiation)
- [Implementation Steps in Detail](#implementation-steps-in-detail)
  - [Step 1: Define the SDF Scene Function](#step-1-define-the-sdf-scene-function)
  - [Step 2: Choose a Difference Method and Implement the Normal Function](#step-2-choose-a-difference-method-and-implement-the-normal-function)
  - [Step 3: Normalize and Apply to Lighting](#step-3-normalize-and-apply-to-lighting)
- [Variant Details](#variant-details)
  - [Variant 1: Reverse Offset Forward Difference](#variant-1-reverse-offset-forward-difference)
  - [Variant 2: Adaptive Epsilon (Distance Scaling)](#variant-2-adaptive-epsilon-distance-scaling)
  - [Variant 3: Large Epsilon Rounding / Anti-Aliasing Trick](#variant-3-large-epsilon-rounding-anti-aliasing-trick)
  - [Variant 4: Anti-Inlining Loop Trick](#variant-4-anti-inlining-loop-trick)
  - [Variant 5: Normal + Edge Detection (Dual-Purpose Sampling)](#variant-5-normal-edge-detection-dual-purpose-sampling)
- [Performance Optimization In-Depth Analysis](#performance-optimization-in-depth-analysis)
  - [Bottleneck 1: SDF Sample Count](#bottleneck-1-sdf-sample-count)
  - [Bottleneck 2: Compile Time Explosion](#bottleneck-2-compile-time-explosion)
  - [Bottleneck 3: Epsilon Selection](#bottleneck-3-epsilon-selection)
  - [Bottleneck 4: Avoiding Redundant Sampling](#bottleneck-4-avoiding-redundant-sampling)
- [Combination Suggestions with Full Code](#combination-suggestions-with-full-code)
  - [1. Normal + Soft Shadow](#1-normal-soft-shadow)
  - [2. Normal + Ambient Occlusion (AO)](#2-normal-ambient-occlusion-ao)
  - [3. Normal + Fresnel Effect](#3-normal-fresnel-effect)
  - [4. Normal + Bump Mapping](#4-normal-bump-mapping)
  - [5. Normal + Triplanar Mapping](#5-normal-triplanar-mapping)
<!-- GENERATED:TOC:END -->
...
- Thus `n = normalize(map(p+εx̂) - map(p), map(p+εŷ) - map(p), map(p+ε�? - map(p))`
...
- The coefficient `0.5773 �?1/�?` normalizes the vertices onto the unit sphere
- The weighted sum `Σ eᵢ·map(p + eᵢ·�?` is equivalent to a gradient estimate in 4 symmetric directions
