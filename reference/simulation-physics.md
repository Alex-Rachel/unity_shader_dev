...
- [Core Principles in Detail](#core-principles-in-detail)
  - [Key Mathematical Tools in Detail](#key-mathematical-tools-in-detail)
- [Implementation Steps in Detail](#implementation-steps-in-detail)
  - [Step 1: Ping-Pong Double Buffer Structure](#step-1-ping-pong-double-buffer-structure)
  - [Step 2: Interaction-Driven (External Force Injection)](#step-2-interaction-driven-external-force-injection)
  - [Step 3: Rendering Layer (Height Field Visualization)](#step-3-rendering-layer-height-field-visualization)
  - [Step 4: Chained Multi-Buffer Iteration (Improving Accuracy)](#step-4-chained-multi-buffer-iteration-improving-accuracy)
  - [Step 5: Texture Data Layout for Particle/Mass-Point Systems](#step-5-texture-data-layout-for-particlemass-point-systems)
  - [Step 6: Spring-Damper Constraint System](#step-6-spring-damper-constraint-system)
  - [Step 7: N-Body Particle Interaction (Biot-Savart Vortex Method)](#step-7-n-body-particle-interaction-biot-savart-vortex-method)
  - [Step 8: State Storage in Specific Pixels (Global Variable Trick)](#step-8-state-storage-in-specific-pixels-global-variable-trick)
- [Common Variant Details](#common-variant-details)
  - [Variant 1: Eulerian Fluid Simulation (Smoke / Ink)](#variant-1-eulerian-fluid-simulation-smoke-ink)
  ...
  - [Variant 4: N-Body Vortex Particle Simulation](#variant-4-n-body-vortex-particle-simulation)
...
- [Performance Optimization Details](#performance-optimization-details)
  - [1. Neighborhood Sampling Optimization](#1-neighborhood-sampling-optimization)
  - [2. N-Body O(N²) Loop Optimization](#2-n-body-on-loop-optimization)
  - [3. Iteration Count vs. Accuracy Balance](#3-iteration-count-vs-accuracy-balance)
  - [4. Adaptive Precision](#4-adaptive-precision)
  - [5. Data Packing Compression](#5-data-packing-compression)
  - [6. Stability Safeguards](#6-stability-safeguards)
- [Combination Suggestions in Detail](#combination-suggestions-in-detail)
  - [1. Physics Simulation + Post-Processing Rendering](#1-physics-simulation-post-processing-rendering)
  - [2. Physics Simulation + SDF Rendering](#2-physics-simulation-sdf-rendering)
  - [3. Physics Simulation + Volume Rendering](#3-physics-simulation-volume-rendering)
  - [4. Multiple Physics System Coupling](#4-multiple-physics-system-coupling)
  - [5. Physics Simulation + Audio Visualization](#5-physics-simulation-audio-visualization)
<!-- GENERATED:TOC:END -->
...
- **ShaderToy-style prototype Multi-Pass Mechanism**: Buffer A/B/C/D read/write between each other, `iChannel0~3` binding, Common pass for shared code
...
The core paradigm of GPU physics simulation is **Buffer Feedback**: leveraging ShaderToy-style prototype's multi-pass architecture to store physical state (position, velocity, density, pressure, etc.) in texture buffers. Each frame reads the previous frame's state, computes new state, and writes it back. Each pixel computes independently in parallel, achieving GPU-level massively parallel physics solving.
...
// IMPORTANT: In Unity URP, getpos/getvel both read from iChannel0 (currentBuf, read-only),
