...
- [Implementation Steps in Detail](#implementation-steps-in-detail)
  - [Step 1: Pseudorandom Number Generator](#step-1-pseudorandom-number-generator)
  ...
  - [Step 4: Material System and BRDF Evaluation](#step-4-material-system-and-brdf-evaluation)
  - [Step 5: Direct Light Sampling (Next Event Estimation)](#step-5-direct-light-sampling-next-event-estimation)
...
- [Common Variants in Detail](#common-variants-in-detail)
  ...
  - [4. Multiple Importance Sampling (MIS)](#4-multiple-importance-sampling-mis)
...
- [Performance Optimization Details](#performance-optimization-details)
  ...
  - [2. Russian Roulette](#2-russian-roulette)
  - [3. Direct Light Sampling (NEE)](#3-direct-light-sampling-nee)
  - [4. Avoiding Self-Intersection](#4-avoiding-self-intersection)
  - [5. Firefly Suppression](#5-firefly-suppression)
  - [6. SDF Scene Optimization](#6-sdf-scene-optimization)
  - [7. High-Quality PRNG](#7-high-quality-prng)
- [Combination Suggestions in Detail](#combination-suggestions-in-detail)
  - [1. Path Tracing + SDF Modeling](#1-path-tracing-sdf-modeling)
  - [2. Path Tracing + Environment Maps](#2-path-tracing-environment-maps)
  - [3. Path Tracing + PBR Materials](#3-path-tracing-pbr-materials)
  - [4. Path Tracing + Volumetric Rendering](#4-path-tracing-volumetric-rendering)
  - [5. Path Tracing + Spectral Rendering](#5-path-tracing-spectral-rendering)
  - [6. Path Tracing + Temporal Accumulation / TAA](#6-path-tracing-temporal-accumulation-taa)
<!-- GENERATED:TOC:END -->
...
- **GLSL basic syntax**: ShaderToy-style prototype multi-pass (Buffer A/B/Image) architecture
...
Accumulation strategy: Store each frame's color and sample count in RGBA (RGB = color accumulation, A = sample count accumulation). Divide by A when displaying to get the average. Clear to zero when `iFrame == 0` to handle ShaderToy-style prototype's edit reset.
...
Use SDF to define complex scene geometry (fractals, smooth boolean operations) while path tracing handles lighting computation. This is the most common combination on ShaderToy-style prototype. SDF's advantage is the ability to easily create shapes difficult to express with traditional meshes (Mandelbulb, Menger sponge, etc.), while path tracing provides physically accurate lighting for these complex geometries.
...
The Disney BRDF/BSDF provides metallic/roughness parameterized material models, combined with GGX microsurface distribution and VNDF importance sampling for production-quality results. In ShaderToy-style prototype, material parameters can be generated procedurally (based on position, noise, etc.).
...
Leverage ShaderToy-style prototype's inter-frame buffer feedback mechanism for progressive rendering. Can be further extended to temporal reprojection, reusing historical frame data during camera movement to accelerate convergence.
