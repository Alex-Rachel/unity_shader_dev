...
- [Prerequisites](#prerequisites)
  - [GLSL Fundamentals](#glsl-fundamentals)
  - [Basic Math](#basic-math)
- [Implementation Steps](#implementation-steps)
  - [Step 1: Establish a Minimal Self-Feedback Loop](#step-1-establish-a-minimal-self-feedback-loop)
  - [Step 2: Implement Self-Advection](#step-2-implement-self-advection)
  - [Step 3: Navier-Stokes Fluid Solver](#step-3-navier-stokes-fluid-solver)
  - [Step 4: Chained Buffers for Accelerated Simulation](#step-4-chained-buffers-for-accelerated-simulation)
  - [Step 5: Separable Gaussian Blur Pipeline](#step-5-separable-gaussian-blur-pipeline)
  - [Step 6: Structured State Storage (Texel-Addressed Registers)](#step-6-structured-state-storage-texel-addressed-registers)
  - [Step 7: Inter-Frame Mouse State Tracking](#step-7-inter-frame-mouse-state-tracking)
- [Variant Details](#variant-details)
  - [Variant 1: Temporal Accumulation Anti-Aliasing (TAA)](#variant-1-temporal-accumulation-anti-aliasing-taa)
  - [Variant 2: Deferred Rendering G-Buffer Pipeline](#variant-2-deferred-rendering-g-buffer-pipeline)
  - [Variant 3: HDR Bloom Post-Processing Pipeline](#variant-3-hdr-bloom-post-processing-pipeline)
...
- [In-Depth Performance Optimization](#in-depth-performance-optimization)
  - [1. Reduce Texture Samples](#1-reduce-texture-samples)
  - [2. Limit Computation Region](#2-limit-computation-region)
  - [3. Reduce Buffer Count](#3-reduce-buffer-count)
  - [4. Reduce Iteration/Sample Count](#4-reduce-iterationsample-count)
  - [5. Initialization Strategy](#5-initialization-strategy)
- [Combination Examples with Complete Code](#combination-examples-with-complete-code)
  - [1. Fluid Simulation + Lighting](#1-fluid-simulation-lighting)
  - [2. Fluid Simulation + Color Advection](#2-fluid-simulation-color-advection)
  - [3. Scene Rendering + Bloom + TAA Post-Processing Chain](#3-scene-rendering-bloom-taa-post-processing-chain)
  - [4. G-Buffer + Screen-Space Effects](#4-g-buffer-screen-space-effects)
  - [5. State Storage + Visualization Separation](#5-state-storage-visualization-separation)
<!-- GENERATED:TOC:END -->
...
- ShaderToy-style prototype execution model: `iChannel0-3` texture inputs, `iResolution`, `iTime`, `iFrame`, `iMouse`
...
- Buffer A/B/C/D concept in ShaderToy-style prototype: each buffer is an independent render pass that outputs to a corresponding texture, which can be read by other passes or itself via iChannel
...
vec4 solveFluid(sampler2D smp, vec2 uv, vec2 w, float time, vec3 mouse, vec3 lastMouse) {
    ...
    // Density update: continuity equation ∂�?∂t + ∇�?ρv) = 0
...
**Why**: Each ShaderToy-style prototype buffer executes only once per frame. By chaining identical code (A reads itself �?B reads A �?C reads B), three iterations are completed in a single frame, significantly increasing simulation speed without adding buffer count. Use the Common tab to avoid code duplication.
...
- Weights [0.05, 0.09, 0.12, 0.15, 0.16, 0.15, 0.12, 0.09, 0.05] approximate a Gaussian distribution with sigma�?.0
...
**Why**: ShaderToy-style prototype does not directly provide mouse velocity. Storing the current frame's `iMouse` in a fixed pixel allows calculating the delta in the next frame. This is critical for fluid interaction �?mouse velocity is needed to apply forces.
