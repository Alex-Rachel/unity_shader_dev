...
- [Step-by-Step Detailed Explanation](#step-by-step-detailed-explanation)
  - [Step 1: SDF Primitive Library](#step-1-sdf-primitive-library)
  - [Step 2: Boolean Operations and Smooth Blending](#step-2-boolean-operations-and-smooth-blending)
  - [Step 3: Scene Definition (map Function)](#step-3-scene-definition-map-function)
  - [Step 4: Raymarching](#step-4-raymarching)
  - [Step 5: Normal Computation](#step-5-normal-computation)
  - [Step 6: Soft Shadows](#step-6-soft-shadows)
  - [Step 7: Ambient Occlusion (AO)](#step-7-ambient-occlusion-ao)
  - [Step 8: Camera and Rendering Pipeline](#step-8-camera-and-rendering-pipeline)
- [Variant Detailed Descriptions](#variant-detailed-descriptions)
  - [Variant 1: Dynamic Organic Body (Smooth Blob Animation)](#variant-1-dynamic-organic-body-smooth-blob-animation)
  - [Variant 2: Infinite Repeating Corridor (Domain Repetition)](#variant-2-infinite-repeating-corridor-domain-repetition)
  - [Variant 3: Character/Creature Modeling (Organic Character Modeling)](#variant-3-charactercreature-modeling-organic-character-modeling)
  - [Variant 4: Symmetry Exploitation](#variant-4-symmetry-exploitation)
  - [Variant 5: PBR Material Rendering Pipeline](#variant-5-pbr-material-rendering-pipeline)
- [Performance Optimization in Detail](#performance-optimization-in-detail)
  - [1. Bounding Volume Acceleration](#1-bounding-volume-acceleration)
  - [2. Per-Object Bounding](#2-per-object-bounding)
  - [3. Adaptive Step Size](#3-adaptive-step-size)
  - [4. Preventing Compiler Inlining](#4-preventing-compiler-inlining)
  - [5. Symmetry Exploitation](#5-symmetry-exploitation)
- [Combination Suggestions in Detail](#combination-suggestions-in-detail)
  - [1. SDF + Noise Displacement](#1-sdf-noise-displacement)
  - [2. SDF + Bump Mapping](#2-sdf-bump-mapping)
  - [3. SDF + Domain Warping](#3-sdf-domain-warping)
  - [4. SDF + Procedural Animation](#4-sdf-procedural-animation)
  - [5. SDF + Motion Blur](#5-sdf-motion-blur)
- [Extended SDF Primitives Reference](#extended-sdf-primitives-reference)
  - [Rounded Box �?`sdRoundBox(vec3 p, vec3 b, float r)`](#rounded-box-sdroundboxvec3-p-vec3-b-float-r)
  - [Box Frame �?`sdBoxFrame(vec3 p, vec3 b, float e)`](#box-frame-sdboxframevec3-p-vec3-b-float-e)
  - [Cone �?`sdCone(vec3 p, vec2 c, float h)`](#cone-sdconevec3-p-vec2-c-float-h)
  - [Capped Cone �?`sdCappedCone(vec3 p, float h, float r1, float r2)`](#capped-cone-sdcappedconevec3-p-float-h-float-r1-float-r2)
  - [Round Cone �?`sdRoundCone(vec3 p, float r1, float r2, float h)`](#round-cone-sdroundconevec3-p-float-r1-float-r2-float-h)
  - [Solid Angle �?`sdSolidAngle(vec3 p, vec2 c, float ra)`](#solid-angle-sdsolidanglevec3-p-vec2-c-float-ra)
  - [Octahedron �?`sdOctahedron(vec3 p, float s)`](#octahedron-sdoctahedronvec3-p-float-s)
  - [Pyramid �?`sdPyramid(vec3 p, float h)`](#pyramid-sdpyramidvec3-p-float-h)
  - [Hex Prism �?`sdHexPrism(vec3 p, vec2 h)`](#hex-prism-sdhexprismvec3-p-vec2-h)
  - [Cut Sphere �?`sdCutSphere(vec3 p, float r, float h)`](#cut-sphere-sdcutspherevec3-p-float-r-float-h)
  - [Capped Torus �?`sdCappedTorus(vec3 p, vec2 sc, float ra, float rb)`](#capped-torus-sdcappedtorusvec3-p-vec2-sc-float-ra-float-rb)
  - [Link �?`sdLink(vec3 p, float le, float r1, float r2)`](#link-sdlinkvec3-p-float-le-float-r1-float-r2)
  - [Plane (arbitrary) �?`sdPlane(vec3 p, vec3 n, float h)`](#plane-arbitrary-sdplanevec3-p-vec3-n-float-h)
  - [Rhombus �?`sdRhombus(vec3 p, float la, float lb, float h, float ra)`](#rhombus-sdrhombusvec3-p-float-la-float-lb-float-h-float-ra)
  - [Triangle (unsigned) �?`udTriangle(vec3 p, vec3 a, vec3 b, vec3 c)`](#triangle-unsigned-udtrianglevec3-p-vec3-a-vec3-b-vec3-c)
- [Deformation Operators Reference](#deformation-operators-reference)
  - [Round �?`opRound(float d, float r)`](#round-oproundfloat-d-float-r)
  - [Onion �?`opOnion(float d, float t)`](#onion-oponionfloat-d-float-t)
  - [Elongate �?`opElongate(vec3 p, vec3 h, vec3 center, vec3 size)`](#elongate-opelongatevec3-p-vec3-h-vec3-center-vec3-size)
  - [Twist �?`opTwist(vec3 p, float k)`](#twist-optwistvec3-p-float-k)
  - [Cheap Bend �?`opCheapBend(vec3 p, float k)`](#cheap-bend-opcheapbendvec3-p-float-k)
  - [Displacement �?`opDisplace(float d, vec3 p)`](#displacement-opdisplacefloat-d-vec3-p)
- [2D-to-3D Constructors Reference](#2d-to-3d-constructors-reference)
  - [Revolution �?`opRevolution(vec3 p, float sdf2d_result, float o)`](#revolution-oprevolutionvec3-p-float-sdf2dresult-float-o)
  - [Extrusion �?`opExtrusion(vec3 p, float d2d, float h)`](#extrusion-opextrusionvec3-p-float-d2d-float-h)
- [Symmetry Operators Reference](#symmetry-operators-reference)
  - [Mirror X �?`opSymX(vec3 p)`](#mirror-x-opsymxvec3-p)
  - [Mirror XZ �?`opSymXZ(vec3 p)`](#mirror-xz-opsymxzvec3-p)
  - [Arbitrary Mirror �?`opMirror(vec3 p, vec3 dir)`](#arbitrary-mirror-opmirrorvec3-p-vec3-dir)
<!-- GENERATED:TOC:END -->
