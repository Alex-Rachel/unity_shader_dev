...
- [Implementation Steps](#implementation-steps)
  - [Step 1: mainSound Entry Point and Basic Framework](#step-1-mainsound-entry-point-and-basic-framework)
  - [Step 2: MIDI Note to Frequency Conversion](#step-2-midi-note-to-frequency-conversion)
  - [Step 3: Basic Oscillators](#step-3-basic-oscillators)
  - [Step 4: Additive Synthesis Instrument](#step-4-additive-synthesis-instrument)
  - [Step 5: FM Synthesis Instrument](#step-5-fm-synthesis-instrument)
  - [Step 6: Percussion Synthesis](#step-6-percussion-synthesis)
  - [Step 7: Note Sequence Arrangement](#step-7-note-sequence-arrangement)
  - [Step 8: Chord Construction](#step-8-chord-construction)
  - [Step 9: Delay and Reverb Effects](#step-9-delay-and-reverb-effects)
  - [Step 10: Beat and Arrangement Structure](#step-10-beat-and-arrangement-structure)
- [Variant Details](#variant-details)
  - [Variant 1: Subtractive Synthesis / TB-303 Acid Synthesizer](#variant-1-subtractive-synthesis-tb-303-acid-synthesizer)
  - [Variant 2: IIR Biquad Filter](#variant-2-iir-biquad-filter)
  - [Variant 3: Vocal / Formant Synthesis](#variant-3-vocal-formant-synthesis)
  - [Variant 4: Algorithmic Composition (Generative Music)](#variant-4-algorithmic-composition-generative-music)
  - [Variant 5: Chord Progression System (Circle of Fifths)](#variant-5-chord-progression-system-circle-of-fifths)
- [Performance Optimization Details](#performance-optimization-details)
- [Combination Suggestions](#combination-suggestions)
  - [1. Combining with Audio Visualization](#1-combining-with-audio-visualization)
  - [2. Combining with Raymarching Scenes](#2-combining-with-raymarching-scenes)
  - [3. Combining with Particle Systems](#3-combining-with-particle-systems)
  - [4. Combining with Post-Processing Effects](#4-combining-with-post-processing-effects)
  - [5. Combining with Text/Graphic Overlays](#5-combining-with-textgraphic-overlays)
<!-- GENERATED:TOC:END -->
...
- **ShaderToy-style prototype Sound Mode**: `vec2 mainSound(int samp, float time)` returns a `vec2` stereo sample value in the range `[-1, 1]`
...
**Why**: ShaderToy-style prototype requires the fixed signature `vec2 mainSound(int samp, float time)`, where the return value's `.x` and `.y` are the left and right channels respectively, with a range of `[-1, 1]`. `samp` is the sample index, and `time` is the corresponding time (in seconds).

```glsl
// ShaderToy-style prototype sound shader basic framework
...
**Why**: Music = timbre × timing. ShaderToy-style prototype has three mainstream arrangement approaches: (A) D() macro accumulation for handwritten melodies, (B) array lookup for complex arrangements, (C) hash pseudo-random for algorithmic composition.
...
6. **Use the Common Pass**: Place constant definitions and shared functions in ShaderToy-style prototype's Common tab, accessible by both Sound and Image, avoiding redundant computation of BPM/SPB, etc.
