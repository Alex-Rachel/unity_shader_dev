...
- [Core Principles](#core-principles)
  - [Cellular Automata (CA)](#cellular-automata-ca)
  - [Reaction-Diffusion (RD)](#reaction-diffusion-rd)
- [Implementation Steps](#implementation-steps)
  - [Step 1: Grid State Storage & Self-Feedback](#step-1-grid-state-storage-self-feedback)
  - [Step 2: Initialization (Noise Seeding)](#step-2-initialization-noise-seeding)
  - [Step 3: Neighbor Sampling & Laplacian](#step-3-neighbor-sampling-laplacian)
  - [Step 4: State Update Rules](#step-4-state-update-rules)
  - [Step 5: Visualization & Coloring](#step-5-visualization-coloring)
- [Complete Code Template](#complete-code-template)
  - [URP Ping-Pong Render Pipeline](#urp-ping-pong-render-pipeline)
  - [Buffer A (Simulation Computation)### Buffer A (Simulation Computation)](#buffer-a-simulation-computation-buffer-a-simulation-computation)
  - [Image (Visualization Output)](#image-visualization-output)
- [Common Variants](#common-variants)
  - [Variant 1: Conway's Game of Life (Discrete CA)](#variant-1-conways-game-of-life-discrete-ca)
  - [Variant 2: Configurable Rule Set CA (B/S Bitmask)](#variant-2-configurable-rule-set-ca-bs-bitmask)
  - [Variant 3: Separable Gaussian Blur RD (Multi-Buffer)](#variant-3-separable-gaussian-blur-rd-multi-buffer)
...
- [Performance & Composition](#performance-composition)
  ...
  - [Composition Directions](#composition-directions)
...
ShaderToy-style prototype setup: Buffer A's iChannel0 = Buffer A (self-feedback, linear filtering). Image's iChannel0 = Buffer A.

### URP Ping-Pong Render Pipeline
Cellular automata and reaction-diffusion belong in a persistent render-target loop in URP.
- Allocate a ping-pong pair for the simulation state.
- Update Buffer A logic in a fullscreen/custom render pass or manager-driven blit step.
- Expose the latest state texture to a visualization pass or to object materials that use the pattern for albedo, emission, displacement, or masks.
- Drive seeding, brush interaction, and parameter presets from C# instead of browser input handlers.
...
void ExecuteStep(CommandBuffer cmd)
{
    simMaterial.SetTexture("_PrevState", currentState);
    simMaterial.SetFloat("_DeltaTime", Time.deltaTime);
    simMaterial.SetVector("_Brush", brushData);
    Blitter.BlitCameraTexture(cmd, currentState, nextState, simMaterial, 0);
    CoreUtils.Swap(ref currentState, ref nextState);
}
```
### Buffer A (Simulation Computation)### Buffer A (Simulation Computation)
```glsl
