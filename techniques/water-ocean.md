...
- [Unity URP Executable](#unity-urp-executable)
  ...
  - [Executable Surface Block](#executable-surface-block)
  - [Validation](#validation)
- [Use Cases](#use-cases)
- [Core Principles](#core-principles)
  - [Wave Generation: Exponential Sine Stacking + Derivative Domain Warping](#wave-generation-exponential-sine-stacking-derivative-domain-warping)
  - [Lighting: Schlick Fresnel + Subsurface Scattering](#lighting-schlick-fresnel-subsurface-scattering)
  - [Water Surface Intersection: Bounded Height Field Marching](#water-surface-intersection-bounded-height-field-marching)
- [Implementation Steps](#implementation-steps)
  - [Step 1: Exponential Sine Wave Function](#step-1-exponential-sine-wave-function)
  - [Step 2: Multi-Octave Wave Stacking with Domain Warping](#step-2-multi-octave-wave-stacking-with-domain-warping)
  - [Step 3: Bounded Bounding Box Ray Marching](#step-3-bounded-bounding-box-ray-marching)
  - [Step 4: Normal Calculation and Distance Smoothing](#step-4-normal-calculation-and-distance-smoothing)
  - [Step 5: Fresnel Reflection and Subsurface Scattering](#step-5-fresnel-reflection-and-subsurface-scattering)
  - [Step 6: Atmosphere and Tone Mapping](#step-6-atmosphere-and-tone-mapping)
- [Complete Code Template](#complete-code-template)
- [Common Variants](#common-variants)
  - [Variant 1: 2D Underwater Caustic Texture](#variant-1-2d-underwater-caustic-texture)
  - [Variant 2: FBM Bump-Mapped Lake Surface](#variant-2-fbm-bump-mapped-lake-surface)
  - [Variant 3: Ridge Noise Coastal Waves](#variant-3-ridge-noise-coastal-waves)
  - [Variant 4: Flow Map Water Animation](#variant-4-flow-map-water-animation)
  - [Variant 5: Beer's Law Water Absorption](#variant-5-beers-law-water-absorption)
...
Use a forward-lit material shader on a mesh surface. Add fullscreen support only if the project specifically needs camera-space water or underwater compositing.

- Base template: `../assets/templates/urp-forward-lit.shader`
- Recipe: `../reference/recipes/water-surface.md`
- Supporting references: `../reference/pipeline/performance.md`, `../reference/pipeline/debugging.md`

### Minimum File Set

- `MyWater.shader`
- Optional `MyWaterController.cs` for wind, wave speed, and foam controls

### Runnable Example Assets

- `../assets/examples/water/OceanSurfaceExample.shader`

### Executable Surface Block
...
- Verify the mesh is stable with `_WaveAmplitude = 0`.
- Increase amplitude gradually and check for lighting continuity.
- Confirm Fresnel or reflection is layered on top of a stable base shading result.
- If depth tint is added, document whether URP opaque texture or depth texture is required.
...
`wave(x) = exp(sin(x) - 1)` �?sharp wave crests (`exp(0)=1`), broad flat troughs (`exp(-2)�?.135`), similar to a trochoidal profile but at much lower computational cost than Gerstner waves.
...
Can be pasted directly into ShaderToy-style prototype to run. Distilled from `afl_ext`'s "Very fast procedural ocean".

```glsl
// Water & Ocean Rendering �?ShaderToy-style prototype Template
