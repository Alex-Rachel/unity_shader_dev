Shader "SkillTemplates/URP/PingPongUpdate"
{
    Properties
    {
        _PreviousState("Previous State", 2D) = "black" {}
        _DeltaTime("Delta Time", Float) = 0.016
        _Dissipation("Dissipation", Range(0.0, 1.0)) = 0.01
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                half4 previous = SAMPLE_TEXTURE2D(_PreviousState, sampler_PreviousState, input.uv);

                half brushDistance = distance(input.uv, _BrushPosition.xy);
                half brushMask = saturate(1.0h - brushDistance / max(_BrushRadius, 1e-4));
                half inject = brushMask * _BrushStrength * _DeltaTime;

                half value = max(previous.r * (1.0h - _Dissipation * _DeltaTime), inject);
